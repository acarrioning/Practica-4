...
var PlayerShip = function() { 
	...
	if(Game.keys['fire'] && this.reload < 0) {
        ...
        if(Game.keys['fwB'] && this.reload < 0) {
	    // Esta pulsada la tecla de disparo y ya ha pasado el tiempo reload
	    Game.keys['fwB'] = false;
	    this.reload = this.reloadTime;
	    // Se añaden al gameboard 2 misiles 
	    this.board.add(new PlayerFireballB(this.x,this.y+this.h/2));
	}

        if(Game.keys['fwN'] && this.reload < 0) {
	    // Esta pulsada la tecla de disparo y ya ha pasado el tiempo reload
	    Game.keys['fwN'] = false;
            Game.PulsoN = true;
	    this.reload = this.reloadTime;
	    // Se añaden al gameboard 2 misiles 
	    this.board.add(new PlayerFireballN(this.x+this.w,this.y+this.h/2));
	}
...
//////////////////////////////////////////////////////FireBall////////////////////////////////////////////
var PlayerFireballN = function(x,y) {
...
PlayerFireballN.prototype.step = function(dt){
    this.x += this.vx * -dt;
    this.y += this.vy * dt;
    this.vy=this.vy+100;                 
    if(this.y < -this.h) { this.board.remove(this); }
};
PlayerFireballN.prototype.draw = function(ctx)  {
    SpriteSheet.draw(ctx,'fireball',this.x,this.y);
};



var PlayerFireballB = function(x,y) {
    this.w = SpriteSheet.map['fireball'].w;
    this.h = SpriteSheet.map['fireball'].h;
    this.x = x - this.w/2; 
...
PlayerFireballB.prototype.step = function(dt)  {
    this.x += this.vx * dt;
    this.y += this.vy * dt;
    this.vy=this.vy+100;            
    if(this.y < -this.h) { this.board.remove(this); }
};
PlayerFireballB.prototype.draw = function(ctx)  {
    SpriteSheet.draw(ctx,'fireball',this.x,this.y);
};
